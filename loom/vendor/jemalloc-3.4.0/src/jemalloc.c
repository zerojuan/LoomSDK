...
malloc_init(void)
{
	// Make sure we have the right sizes for our types.
	assert(1<<LG_SIZEOF_PTR == sizeof(void*));
	assert(1<<LG_SIZEOF_INT == sizeof(int));
	assert(1<<LG_SIZEOF_LONG == sizeof(long));
	assert(1<<LG_SIZEOF_INTMAX_T == sizeof(intmax_t));
